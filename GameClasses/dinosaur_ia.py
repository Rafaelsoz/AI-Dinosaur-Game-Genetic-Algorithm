...
from constants import PINK_DINO_RUN, PINK_DINO_JUMP, PINK_DINO_DOWN
from constants import PURPLE_DINO_RUN, PURPLE_DINO_JUMP, PURPLE_DINO_DOWN
import numpy as np
import pygame
...
class DinosaurIA:
    def __init__(self, x_pos: int = 200, color: str = "grey"):
        ...
        if color == "grey":
            ...
            self.down_img = PINK_DINO_DOWN
            self.run_img = PINK_DINO_RUN
            self.jump_img = PINK_DINO_JUMP
        elif color == "purple":
            self.down_img = PURPLE_DINO_DOWN
            self.run_img = PURPLE_DINO_RUN
            self.jump_img = PURPLE_DINO_JUMP
    ...
    def ia_move(self, key):
        # 0 = Jump, 1 = Down
        if self.down and self.dino_rect.y <= self.y_pos_down:
            self.down_dinosaur()
