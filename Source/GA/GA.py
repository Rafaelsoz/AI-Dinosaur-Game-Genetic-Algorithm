...
class GA:
    def __init__(self, num_generations, population_size, target, rate_mutation: float = 0.005,
                 start_rate_crossing: float = 0.8, start_rate_mutation: float = 0.2, decimal_place: int = 4,
                 number_of_elitists: int = 4, inferior_limit_pos: int = 80, upper_limit_pos: int = 220):
    ...
    def reproduction(self, father_nn, father_pos, mother_nn, mother_pos) -> (DinosaurIA, DinosaurIA):
        ...
        first_son = DinosaurIA(x_pos=first_son_pos, color="green")
        first_son.neurons = first_son_nn

        second_son = DinosaurIA(x_pos=second_son_pos, color="red")
    ...
    def create_new_population(self):
        ...
        for elit in self.elitists:
            new_dinosaur = DinosaurIA(x_pos=elit.x_pos, color="orange")
